...
class WishListFragment : BaseFragment(), IRealEstateListener, IToolbarListener,
...
//            }
    ...
    private fun getRealEstateList() {
        try {
            ...
            userId?.let { realEstateViewModel.getPropertyList(it,ApiUrlEndPoints.GET_WISHLIST) }
        } catch (e: Exception) {
            e.printStackTrace()
        }
    }
    ...
    override fun setFilterView() {
        try {
            val modalbottomSheetFragment = SortAndFilterScreenWishList(this)
